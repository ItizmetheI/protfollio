    ...
    <PageTransition className="pt-32 md:pt-48 pb-24 px-6 md:px-12 relative z-10 flex flex-col bg-neutral-950">
      ...
      >
        <h1 className="text-5xl md:text-8xl font-display font-medium tracking-tighter leading-none mb-12 uppercase">
          CRAFT & <br /><span className="italic text-rose-400 rgb-hover">DISCIPLINE</span>
        </h1>
        
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-32">
          <div className="w-full lg:w-1/3">
            <h2 className="text-2xl uppercase tracking-widest text-rose-500 font-sans font-bold mb-8 flex items-center gap-4">
            ...
            <ul className="flex flex-col gap-6 text-xl tracking-tight font-display">
              <li className="flex justify-between border-b border-neutral-800 pb-4 text-white group cursor-default">
                <span className="group-hover:text-rose-400 transition-colors">Creative Frontend Dev</span>
              </li>
              <li className="flex justify-between border-b border-neutral-800 pb-4 text-neutral-300 group cursor-default">
                <span className="group-hover:text-purple-400 transition-colors">UI/UX & Web Design</span>
              </li>
              <li className="flex justify-between border-b border-neutral-800 pb-4 text-neutral-300 group cursor-default">
                <span className="group-hover:text-indigo-400 transition-colors">Performance Optimization</span>
              </li>
              <li className="flex justify-between border-b border-neutral-800 pb-4 text-neutral-300 group cursor-default">
                <span className="group-hover:text-rose-500 transition-colors">Motion & WebGL</span>
              </li>
            </ul>
          </div>
          ...
          <div className="px-6 md:px-12 py-16 flex flex-wrap gap-12 justify-between relative z-10">
            {STATS.map((stat, i) => (
                <div key={i} className="flex flex-col items-center md:items-start group">
                  <span className="text-5xl md:text-7xl font-display font-medium tracking-tighter text-white mb-2 group-hover:scale-110 transition-transform origin-left text-transparent bg-clip-text bg-gradient-to-b from-white to-neutral-500">
                    <AnimatedCounter value={stat.value} duration={2 + i * 0.5} />
                  </span>
      ...
      <section className="max-w-7xl mx-auto w-full mb-32">
          <div className="mb-16">
            <h2 className="text-3xl md:text-5xl font-display font-medium tracking-tighter uppercase mb-4">Our <span className="italic text-rose-400 rgb-hover">Process</span></h2>
            ...
            {PROCESS.map((p, i) => (
              <Magnetic key={i} strength={0.02}>
                <motion.div 
                  ...
                  className="flex flex-col group p-10 rounded-3xl bg-neutral-900 border border-neutral-800 hover:border-rose-500/30 transition-colors h-full"
                >
                    <div className="text-rose-500/20 text-6xl font-display font-bold tracking-tighter mb-6 group-hover:text-rose-500 transition-colors duration-500">{p.step}</div>
      ...
      <section className="max-w-7xl mx-auto w-full relative">
          ...
          <div className="relative z-10 flex flex-col items-center pt-12">
            <Quote size={64} className="text-rose-500/40 mb-12" />
            
            <div className="flex overflow-x-auto snap-x snap-mandatory hide-scrollbar w-full pb-8 cursor-grab active:cursor-grabbing">
                <div className="flex gap-8 px-4 w-max h-full">
                  {TESTIMONIALS.map((testimonial, i) => (
                    <motion.div 
                      key={i}
                      className="snap-center w-[85vw] md:w-[600px] shrink-0 bg-neutral-950 border border-neutral-800 p-8 md:p-12 rounded-3xl flex flex-col justify-between"
                      whileHover={{ y: -5 }}
                      transition={{ duration: 0.3 }}
                    >
                        <p className="text-xl md:text-2xl font-sans font-light leading-relaxed mb-12 text-neutral-300">"{testimonial.text}"</p>
                        <div className="flex flex-col">
                          <span className="font-display font-medium text-white text-xl">{testimonial.name}</span>
                          <span className="text-rose-400 text-xs font-sans font-bold uppercase tracking-widest mt-2">{testimonial.role}</span>
                        </div>
                    </motion.div>
                  ))}
                </div>
